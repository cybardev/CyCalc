...
def validate_input_request_fields(data_request, required_fields):
    ...
    return (not data_request) or (
        not all(field in data_request for field in required_fields)
    )
...
def divide():
    ...
    if number_2 == 0:
        return (
            jsonify({"error": "Division by zero"}),
            400,
        )  # Return 'error' if division is by 'zero'
    result = number_1 / number_2
    return jsonify({"result": result})
...
def square_root():
    ...
    if number < 0:
        return (
            ...
            400,
        )  # Return 'error' if calculating square root of a negative number
...
def logarithm():
    ...
    if number <= 0:
        return (
            jsonify({"error": "Cannot calculate logarithm of a non-positive number"}),
            400,
        )
